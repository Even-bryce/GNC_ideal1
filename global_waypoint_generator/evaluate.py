...
def evaluate(model_path, data_dir, map_dim, cluster_eps=0.02, peak_radius=0.02):
    ...
    with torch.no_grad():
        for i, (points, targets, mask, gt_waypoints_list) in enumerate(test_loader):
            ...
            probs_wp = probs[..., 1]        # [B, N]
            targets_tube = targets[..., 0]  # [B, N]
            targets_wp = targets[..., 1]    # [B, N]

            # ==========================================
            # 💡 2. 统计航路点分类指标 (使用 probs_wp 和 targets_wp)
            # ==========================================
            if mask.sum() > 0:
                val_max_probs.append(probs_wp[mask].max().item())
            ...
            if points_trans.shape[1] >= 6:  
                ...
                if mask_SE.sum() > 0:
                    val_se_mean_probs.append(probs_wp[mask_SE].mean().item())
                if mask_MID.sum() > 0:
                    mid_probs = probs_wp[mask_MID]
                    val_mid_mean_probs.append(mid_probs.mean().item())
                    # ⚠️ 注意：这里评估时还是用 0.5 来看漏报
            ...
            target_wp_raw = targets_wp[0, :]
            prob_tube_raw = probs_tube[0, :]
            prob_wp_raw = probs_wp[0, :]
            mask_vis = mask[0, :] 

            xyz_vis = xyz_vis_raw[mask_vis]
            target_wp_vis = target_wp_raw[mask_vis]
            prob_tube_vis = prob_tube_raw[mask_vis]
            prob_wp_vis = prob_wp_raw[mask_vis]

            start_pt = xyz_vis[0].cpu().numpy()
            goal_pt = xyz_vis[1].cpu().numpy()

            mid_xyz = xyz_vis[2:]
            mid_target_wp = target_wp_vis[2:]
            mid_prob_tube = prob_tube_vis[2:]
            mid_prob_wp = prob_wp_vis[2:]
            ...
            # ==========================================
            # 💡 4. 终极大招：联合推理预测提取 (Joint Inference)
            # ==========================================
            # 既要在管道安全区内 (tube > 0.5)，又要满足航路点置信度极高 (wp > 0.8)
            # pred_mask_vis = (mid_prob_tube > 0.5) & (mid_prob_wp > 0.8)
            pred_mask_vis = mid_prob_tube > 0.5
            
            pred_xyz_filtered = mid_xyz[pred_mask_vis].cpu().numpy()
            pred_scores_filtered = mid_prob_wp[pred_mask_vis].cpu().numpy()
            ...
            gt_mask_vis = mid_target_wp > 0.8
            gt_xyz_filtered = mid_xyz[gt_mask_vis].cpu().numpy()
            gt_scores_filtered = mid_target_wp[gt_mask_vis].cpu().numpy()
            ...
            if 0 <= i <= 10:
                visualize_result(
                    xyz_vis, target_wp_vis, prob_wp_vis,  # 这里传入 WP 通道给可视化函数画点
