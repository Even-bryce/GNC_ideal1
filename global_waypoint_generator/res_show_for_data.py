...
def plot_map_pv(env_map):
    ...
    # 添加地图边界框（新 API）
    x_size = env_map["map_dim"][0]  # 例如 1500
    y_size = env_map["map_dim"][1]  # 例如 1500
    z_size = env_map["map_dim"][2]    # 例如 300
    bounds = [0, x_size, 0, y_size, 0, z_size]
...
def plot_tree_and_path_and_waypoints(env_map, node_list=None, path=None, waypoint_list=None):
    """
    使用 PyVista 绘制：
    - 障碍物
    - RRT 树
    ...
    if node_list is not None and len(node_list) > 1:
        ...
        if line_segments:
            line_segments = np.array(line_segments, dtype=float)
            n_lines = len(line_segments)
            ...
            tree_mesh = pv.PolyData(flat_points, lines=cells)
            tree_tubes = tree_mesh.tube(radius=0.2)
    ...
    if waypoint_list is not None and len(waypoint_list) > 0:
        ...
        if len(wp) > 1:
            ...
            wp_mesh = pv.PolyData(flat_points, lines=cells)
            wp_tubes = wp_mesh.tube(radius=1.2)

            plotter.add_mesh(wp_tubes, color='purple', label='Waypoint Route')
