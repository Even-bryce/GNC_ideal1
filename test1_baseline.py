    ...
    for test_id in range(n_tests):
        ...
        if planner_type == 'RRT_star':
            ...
            rrt = RRT_star_Bi(env_map=env_map, 
                            ...
                            search_radius=search_radius, 
                            search_until_max_iter=True)
        elif planner_type == 'RRT_star_Bi_Bias':
            rrt = RRT_star_Bi_Bias(env_map=env_map, 
                            start=waypoints[0], 
                            goal=waypoints[-1], 
                            R_crash=r_agent_crash, 
                            R_risk=r_agent_risk,
                            ...
                            search_radius=search_radius, 
                            search_until_max_iter=True)
        elif planner_type == 'RRT_star_Bi_APF':
            rrt = RRT_star_Bi_APF(env_map=env_map, 
                            start=waypoints[0], 
                            goal=waypoints[-1], 
                            R_crash=r_agent_crash, 
                            R_risk=r_agent_risk,
            ...
            rrt = RRT_star_DBVSB_APF_Informed(env_map=env_map, 
                            ...
                            search_radius=search_radius, 
                            search_until_max_iter=True)
        elif planner_type == 'RRT_star_Bi_Bias_APF':
            rrt = RRT_star_Bias_APF_Bi(env_map=env_map, 
                            start=waypoints[0], 
                            goal=waypoints[-1], 
                            R_crash=r_agent_crash, 
                            R_risk=r_agent_risk,
                            ...
                            search_radius=search_radius, 
                            search_until_max_iter=True)
        elif planner_type == 'RRT_star_Bi_Bias_APF_Informed':
            rrt = RRT_star_Bias_APF_Bi_Informed(env_map=env_map, 
                            start=waypoints[0], 
                            goal=waypoints[-1], 
                            R_crash=r_agent_crash, 
                            R_risk=r_agent_risk,
            ...
            rrt = VRRT_star_Bi(env_map=env_map, 
                            ...
                            search_radius=search_radius, 
                            search_until_max_iter=True)
        elif planner_type == 'VRRT_star_Bi_APF':
            rrt = VRRT_star_Bi_APF(env_map=env_map, 
                            waypoints=waypoints,
                            R_crash=r_agent_crash, 
                            R_risk=r_agent_risk,
                            obstacle_list=obstacles, 
                            ...
                            search_radius=search_radius, 
                            search_until_max_iter=True)
        elif planner_type == 'VRRT_star_Bi_Bias':
            rrt = VRRT_star_Bi_Bias(env_map=env_map, 
                            waypoints=waypoints,
                            R_crash=r_agent_crash, 
                            R_risk=r_agent_risk,
                            obstacle_list=obstacles, 
                            ...
                            search_radius=search_radius,
                            search_until_max_iter=True)
        elif planner_type == 'VRRT_star_Bi_Bias_APF':
            rrt = VRRT_star_Bi_Bias_APF(env_map=env_map, 
                            waypoints=waypoints,
                            R_crash=r_agent_crash, 
                            R_risk=r_agent_risk,
                            obstacle_list=obstacles, 
            ...
            rrt = VRRT_star_Bi_Bias_APF_Informed(env_map=env_map, 
                                        ...
                                        search_radius=search_radius, 
                                        search_until_max_iter=True)
        elif planner_type == 'VRRT_star_Bi_Informed_APF':
            rrt = VRRT_star_Bi_Informed_APF(env_map=env_map, 
                                        waypoints=waypoints,
                                        R_crash=r_agent_crash, 
                                        R_risk=r_agent_risk,
                                        obstacle_list=obstacles, 
...
if __name__ == '__main__':
    ...
    for res in results:
        ...
        print(f"{planner_type_list[len(success_rates)-1]:25s} | 成功: {succ}/{num_of_tests} | 成功率: {sr:.1f}%")
    ...
    def print_metric_table(title, headers, planners, columns, col_formats):
        ...
        for i, planner in enumerate(planners):
            ...
            for idx, col in enumerate(columns):
                val = col[i]
                # 使用对应的格式化函数（注：col_formats长度必须与columns一致）
