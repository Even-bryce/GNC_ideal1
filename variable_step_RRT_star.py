...
if __name__ == '__main__':
    ...
    tasks = [([0,0,0],[1500, 1500, 50])] # 对角线起终点
    ...
    # 创建 expand_dis 列表
    expand_dis_values = list(range(expand_dis_start, expand_dis_end + 1, expand_dis_step))
    all_results = []
    for expand_dis in expand_dis_values:
        ...
        print(f"测试完成 expand_dis = {expand_dis}, 成功率: {env_success_rate:.1f}%")
            
    # 4. 计算平均值
    print("-" * 80)
    print(f"{'Step':<4} | {'Rate(%)':<4} | {'Iter':<6} | {'Time_first':<6}  | {'Length_first':<6} | {'Time_final':<6} | {'Length_final':<6}")
    print("-" * 80)

    for res in all_results:
        print(f"{res['expand_dis']:<4} | {res['success_rate']:<7.1f} | {res['avg_iters']:<6.1f} | {res['avg_time_first']:<11.4f} | {res['avg_length_first']:<12.1f} | {res['avg_time_final']:<10.4f} | {res['avg_length_final']:<10.1f}")
