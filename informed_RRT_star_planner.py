    ...
    def planning(self):
        ...
        for i in range(self.max_iter):  # 循环执行最大迭代次数
            ...
            if (not self.check_collision(new_node) and 
                ...
                if not first_path_found:
                    ...
                    if potential_goal_ind is not None:
                        ...
                        self.use_informed_sampling = True  # 启用 Informed 采样
                        ...
                        self.c_best = self.node_list[potential_goal_ind].cost + self.calc_distance(self.node_list[potential_goal_ind], self.goal)
            ...
            if temp_goal_ind is not None:
                ...
                temp_path_len = calculate_path_length(temp_path_coords)
