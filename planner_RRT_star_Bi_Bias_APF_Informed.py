...
class RRT_star_Bias_APF_Bi_Informed:
        ...
        for i in range(self.max_iter):
            ...
            if not first_path_found:
                if root_a is self.start:
                    rnd = self.sample_goal(20, self.goal)
                else:
                    rnd = self.sample_goal(20, self.start)
            else:
    ...
    def sample_goal(self, goal_sample_rate, goal_node):
        """
        根据给定的采样率决定是否采样目标点
        ...
        if random.randint(0, 100) > goal_sample_rate:
            ...
            return [goal_node.x, goal_node.y, goal_node.z]
...
if __name__ == '__main__':
    ...
    for i, (start, goal) in enumerate(tasks):
        ...
        for j in range(num_of_tests):
            # 初始化 RRT*
            rrt_star = RRT_star_Bias_APF_Bi_Informed(
                start=start, 
                goal=goal, 
